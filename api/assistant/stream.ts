...
export function assistantStreamHandler(ai: GoogleGenAI) {
  ...
  const prompt = `You are a calendar assistant. Track and remember details of the user's calendar across turns (do NOT repeat full lists unless asked). Always interpret dates/times in the user's timezone (${userTz}). If you create or update events without explicit timezone, use the user's timezone and convert to ISO with offset (use 'Z' only if UTC explicitly requested). Avoid hallucinating attendees or locations. Current events (summarize mentally, do not echo fully): ${JSON.stringify(events)}\n\n${actionSchema}\nProvide an assistant reply. Conversation history:\n\n`;
